...
declare @minStudentEnrolledDate date
set @minStudentEnrolledDate = (select min(EnrolledDate) from [dbo].[Student])
update [dbo].[Teacher]
set
	HireDate = DATEADD(day, (-1) * (111 + ABS(CHECKSUM(NewID())) % 333 + DATEDIFF(day, @minStudentEnrolledDate, HireDate)), HireDate)
,	DateOfBirth = DATEADD(day, (-1) * (111 + ABS(CHECKSUM(NewID())) % 333 + DATEDIFF(day, @minStudentEnrolledDate, HireDate)), DateOfBirth)
from
	[dbo].[Teacher]
where
	ID <= 41
and HireDate > @minStudentEnrolledDate
GO
...
add constraint df_value default 100 for AchievementMaxPoints
...
add constraint Unique_Name unique (AchievementType)

alter table Student
add constraint [FK_FirstName] foreign key([FirstName])
references [dbo].[Teacher] ([FirstName])
go

select course.Name as CourseName from dbo.Course
cross join dbo.AchievementType Name
go

select teacher.FirstName as TeacherName from dbo.Teacher 
cross join dbo.Grade Grade
go

select teacher.FirstName as TeacherName from dbo.Teacher 
inner join dbo.Grade on teacher.FirstName = grade.GRADE
go

select teacher.FirstName as TeacherName from dbo.Teacher 
right join dbo.Grade on teacher.FirstName = grade.GRADE
go